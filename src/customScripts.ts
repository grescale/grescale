...
async function validateRecordInput(
  ...
  for (const [fieldName, value] of Object.entries(data || {})) {
    ...
    // Skip if field not defined in schema (will be handled by DB)
    ...
    switch (fieldType) {
      ...
      case "date_only":
        ...
        // Basic validation for date-only format
      ...
      case "file":
        // File fields are typically paths or references
        if (typeof value !== "string") {
          throw makeHttpError(
            400,
...
customRouter.use("/api/*", async (c, next) => {
  const pathname = new URL(c.req.url).pathname;
  const matched = findMatchingEndpoint(c.req.method.toUpperCase(), pathname);

  if (!matched) return next();

  try {
    const result = await matched.endpoint.handler(
      await buildHookContext(c, matched.params),
    );
    return normalizeHandlerResult(c, result);
  } catch (error: any) {
    console.error(
      `Custom endpoint failed (${matched.endpoint.fileName}):`,
      error,
    );
    const status =
